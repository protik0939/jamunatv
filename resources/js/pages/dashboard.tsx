...
export default function Dashboard() {
    ...
    return (
        <AppLayout breadcrumbs={breadcrumbs}>
            ...
            <div className="flex h-full flex-1 flex-col gap-4 rounded-xl p-4 overflow-x-auto">
                <div className="grid auto-rows-min gap-4 md:grid-cols-3">
                    ...
                    <div className="relative aspect-video rounded-xl border border-sidebar-border/70 dark:border-sidebar-border  overflow-auto">
                        ...
                        <Flex gap="3">
                            <Button className='!cursor-pointer' color="crimson" variant="soft" onClick={() => setShowModal('updateAuthors')}>
                                Add New Authors
                            </Button>
                        </Flex>
                        <div className='flex flex-col w-full items-center px-2'>
                            {
                                currentAuthors.map(c => (
                                    <Box className='w-full py-1.5' key={c.authors_id}>
            ...
            <div className={`w-full h-screen flex items-center justify-center fixed ${showModal == 'updateNews' ? 'block' : 'hidden'}`}>
                <div className='w-full h-full flex justify-center items-center'>
                    <form onSubmit={updateAuthor} className='w-1/2 h-2/3 bg-black/50 backdrop-blur-3xl rounded-2xl p-10 flex justify-center items-center flex-col space-y-2'>
                        <h1>Add News</h1>
                        <Flex direction="column" gap="3" maxWidth="250px">
                            <TextField.Root color="red" variant="soft" placeholder="Title" onChange={(e) => setAuthorsUpload(prev => ({ ...prev, name: e.target.value }))} />
                            <TextField.Root color="red" variant="soft" placeholder="Description" onChange={(e) => setAuthorsUpload(prev => ({ ...prev, email: e.target.value }))} />
                            <TextField.Root color="red" variant="soft" placeholder="Author" onChange={(e) => setAuthorsUpload(prev => ({ ...prev, designation: e.target.value }))} />
                            <TextField.Root color="red" variant="soft" placeholder="Categories" onChange={(e) => setAuthorsUpload(prev => ({ ...prev, designation: e.target.value }))} />
                            <Button color="crimson" variant="soft" type='submit'>
                                Submit
                            </Button>
                            <Button type='button' onClick={() => setShowModal('none')}>Close</Button>
